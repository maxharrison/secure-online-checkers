...
gameTree :: Board -> Player -> Route -> Tree (Board, Player, Route)
gameTree b p r = Node (b, p, r) [gameTree b' (next p) r | (b', r) <- moves b p]


bestMove :: Int -> (Board -> Int) -> Board -> Player -> Maybe (Board, Player, Route)
bestMove depth sf b p = if null moves then Nothing else Just (random moves)
    where moves = [bpr | Node (bpr, s) _ <- ts, s == best]
          tree = prune depth (gameTree b p er)
